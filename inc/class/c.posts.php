  ...
  /*
	...
	*/
    ...
    $postData[post_body] = preg_replace('/\[url=(.*?)\](.*?)\[\/url\]/is', $replacement, $postData[post_body]);
        ...
        // DESC
    $postData['post_desc'] = htmlentities(stripslashes(strip_tags($postData['post_body'])));
    
    $postData['post_desc'] = substr($postData['post_desc'], 0, 170) ;
	...
	*/
    ...
    function topcat(){
    global $tsCore, $tsUser;
    $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT c.cid, c.c_nombre,c.c_seo, c.c_img, COUNT(p.post_id) AS total, p.post_category FROM p_categorias AS c LEFT JOIN p_posts AS p ON p.post_category = c.cid WHERE post_status = 0 GROUP BY c.c_nombre ORDER BY total DESC');
    $data = result_array($query);
	...
	*/
	function getRelated($tags){
		...
		//
		$query = db_exec(array(__FILE__, __LINE__), 'query', "SELECT DISTINCT p.post_id, p.post_title, p.post_portada,  p.post_comments, p.post_puntos, p.post_hits, p.post_favoritos, p.post_seguidores, p.post_category, p.post_private, c.c_seo, c.c_img FROM p_posts AS p LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE MATCH (post_tags) AGAINST ('$tags' IN BOOLEAN MODE) AND p.post_status = 0 AND post_sticky = 0 ORDER BY rand() LIMIT 0,10");
		//
		$data = result_array($query);
...
$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT user_id, user_name,user_activo FROM u_miembros WHERE user_activo= \'1\' AND user_baneado= \'0\' ORDER BY user_id DESC LIMIT 1');
...
return $data;
        ...
        if($data['post_user']){
            if($data['post_block_comments'] != 1 || $data['post_user'] == $tsUser->uid || $tsUser->is_admod || $tsUser->permisos['mocepc']){
				...
				if(db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `p_comentarios` (`c_post_id`, `c_user`, `c_date`, `c_body`, `c_ip`) VALUES (\''.(int)$post_id.'\', \''.$tsUser->uid.'\', \''.$fecha.'\', \''.$comentario.'\', \''.$_SERVER['REMOTE_ADDR'].'\')')) {
                    ...
                    //notificamos si mencionaron en un post  tutan-kabron
    ...
    */
    ...
    //menciones notificacion tutan-kabron
