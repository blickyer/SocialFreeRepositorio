...
class tsCuenta {
    ...
    */
    private function unData($data){
		...
		$data['p_socials']['sk'] = $data['p_socials'][4];
        //
	...
	*/
	function loadHeadInfo($user_id){
		...
		// INFORMACION GENERAL
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT u.user_id, u.user_name, u.user_registro, u.user_last_ip , u.user_referidos, u.user_lastactive, u.user_activo, u.user_baneado, p.user_sexo, p.p_fondo, p.user_pais, p.p_nombre, p.p_avatar, p.p_mensaje, p.p_socials, p.p_fondoper, p.p_empresa, p.p_configs, p.p_steam, p.p_skype FROM u_miembros AS u, u_perfil AS p WHERE u.user_id = \''.(int)$user_id.'\' AND p.user_id = \''.(int)$user_id.'\'');
		...
		$data['visitas'] = result_array(db_exec(array(__FILE__, __LINE__), 'query', 'SELECT v.*, u.user_id, u.user_name FROM w_visitas AS v LEFT JOIN u_miembros AS u ON v.user = u.user_id WHERE v.for = \''.(int)$user_id.'\' && v.type = \'1\' && user > 0 ORDER BY v.date DESC LIMIT 21'));
		...
		// REAL STATS
		$data['stats'] = db_exec('fetch_assoc', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT u.user_id, u.user_rango, u.user_puntos, u.user_posts, u.user_comentarios, u.user_seguidores, u.user_cache, r.r_name, r.r_color, u.uid_act, u.dinok, v.post_user, v.x_dinero, v.post_status, v.p_validate FROM u_miembros AS u LEFT JOIN u_rangos AS r ON  u.user_rango = r.rango_id LEFT JOIN p_posts AS v ON v.post_user = u.user_id WHERE u.user_id = \''.(int)$user_id.'\''));
	...
	function savePerfil(){
		...
		switch($save){
			...
			case 2:
				...
				$steam = $tsCore->setSecure($tsCore->parseBadWords($_POST['steam']), true);
				...
				$perfilData = array(
					...
					'socials' => serialize(array($facebook,$twitter,$youtuber)),
					'steam' => $tsCore->setSecure($tsCore->parseBadWords($_POST['steam']), true),
					'skype' => $tsCore->setSecure($tsCore->parseBadWords($_POST['skype']), true),
            ...
            case 7:
                ...
                $array = array('m' => $_POST['muro'], 'mf' => $muro_firm, 'rmp' => $rec_mps, 'hits' => $see_hits, 'pvis' => $see_pvis);
                //
