    ...
    */
    function saveConfig()
    {
        ...
        $c = array(
            ...
            'keywords' => $tsCore->setSecure($tsCore->parseBadWords($_POST['keywords'])),
        ...
        if (db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE `w_configuracion` SET `titulo` = \'' . $c['titulo'] . '\', `slogan` = \'' .
         ...
         $c['slogan'] . '\', `keywords` = \'' . $c['keywords'] . '\', `descripcion` = \'' .

$c['descripcion'] . '\', `url` = \'' . $c['url'] . '\', `mobile` = \'' . $c['mobile'] . '\', `chat_id` = \'' . $c['chat'] .
            ...
            '\', `offline_message` = \'' . $c['offline_message'] . '\', `p_p_refs` = \'' . $c['p_p_refs']. '\', `dinerox` = \'' . $c['dinerox'] .'\', `dinerp` = \'' . $c['dinerp'] . '\' WHERE `tscript_id` = \'1\''))
            return true;
        else
...
return '<div style="width:655px;" class="dinerok">Operaci&oacute;n Exitosa.</div>';
