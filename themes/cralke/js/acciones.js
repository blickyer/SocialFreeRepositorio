...
function print_editor() {
	//Editor de posts
	if($('#markItUp') && !$('.wysibb-texarea').length || $('#wysibb') && !$('.wysibb-texarea').length){
		$('#markItUp, #wysibb').wysibb();
		$('#moreemofn, #emoticons').remove();
	}
	//Editor de posts comentarios
	if($('#body_comm') && !$('.wysibb-texarea').length){
		var wbbOpt = { buttons: "smilebox,|,color,|,quote,|,bold,|,italic,|,underline,|,strike,|,hr,|,spoiler,|,img,|,video,|,link,|,messages" }
		$('#body_comm').removeAttr('onblur onfocus class style title').css('height', '80').html('').wysibb(wbbOpt);
	}
	//Editor de respuestas en mensajes
	if($('#respuesta') && !$('.wysibb-texarea').length){
...
show: function(class_aux){
	...
	if(this.close_button)
		$('#mydialog #dialog').append('<img class="close_dialog" src="'+ global_data.img +'mega/close.png" onclick="mydialog.close()" />');
	else
