							...
							<div id="div_cmnt_{$c.cid}" class="{if $tsPost.autor == $c.c_user}especial1{elseif $c.c_user == $tsUser->uid}especial3{/if} P_comments">
								...
								<div class="comentario-post clearbeta">
									...
									<div class="comment-box" id="pp_{$c.cid}" {if $c.c_status == 1 || !$c.user_activo && $tsUser->is_admod}style="opacity:0.5"{/if} >                                        
										<div class="comment-info clearbeta">                                        	
											<div class="floatL">                                            	
												<a style="color:#{$c.r_color};" href="{$tsConfig.url}/perfil/{$c.user_name}" class="nick">{$c.user_name}</a> {if $tsUser->is_admod}(<span style="color:red;">IP:</span> <a href="{$tsConfig.url}/moderacion/buscador/1/1/{$c.c_ip}" class="geoip" target="_blank">{$c.c_ip}</a>){/if} dijo
