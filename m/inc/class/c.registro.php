	...
	function registerUser(){
		...
		foreach($tsData as $key => $val){
			if($val == '') {
				...
				return $key_error.': El campo'.$key_error.' es requerido';
			}
		}	
		
		//HACEMOS COMPROBACIONES DE DATOS DE NUEVO
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT `user_name` FROM `u_miembros` WHERE LOWER(user_name) = \''.$tsCore->setSecure($tsData['user_nick']).'\' LIMIT 1');
		if(db_exec('num_rows', $query) > 0) return '0: El nombre de usuario ya se encuentra registrado';
		//
		if(!filter_var($tsData['user_email'], FILTER_VALIDATE_EMAIL)) return '0: Coloca un email v&aacute;lido (ejemplo@kmario.com)';
		//
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT `user_email` FROM `u_miembros` WHERE LOWER(user_email) = \''.$tsCore->setSecure($tsData['user_email']).'\' LIMIT 1');
		if(db_exec('num_rows', $query) > 0) return '0:  El email ya se encuentra registrado';
		//
		if(db_exec('num_rows', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT id FROM w_blacklist WHERE (type = \'3\' && value = \''.$tsCore->setSecure(strstr($tsData['user_email'], '@')).'\') ||  (type = \'4\' && value = \''.$tsCore->setSecure(strstr($tsData['user_email'], '@', true)).'\') || (type = \'4\' && value = \''.$tsCore->setSecure($tsData['user_nick']).'\') LIMIT 1'))) $valid = '0: Email o nick no permitido.';
		//
		if($tsCore->settings['c_reg_active'] == 0) return '0: No se le permite el registro en el sitio.';
		// CAPTACHA
		...
		if(!$robot->is_valid) return 'recaptcha: Lo escribiste mal animal!';
		...
		if(db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `u_miembros` (`user_name`, `user_password`, `user_email`, `user_rango`, `user_registro`) VALUES (\''.$tsCore->setSecure($tsData['user_nick']).'\', \''.$tsCore->setSecure($key).'\', \''.$tsCore->setSecure($tsData['user_email']).'\', '.(empty($tsCore->settings['c_reg_rango']) ? 3 : $tsCore->settings['c_reg_rango']).', \''.$tsData['user_registro'].'\')')){
			...
			db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `u_perfil` (`user_id`, `user_dia`, `user_mes`, `user_ano`, `user_pais`, `user_estado`, `user_sexo`) VALUES (\''.(int)$tsData['user_id'].'\', \''.(int)$tsData['user_dia'].'\', \''.(int)$tsData['user_mes'].'\', \''.(int)$tsData['user_anio'].'\', \''.$tsCore->setSecure($tsData['user_pais']).'\', \''.$tsCore->setSecure($tsData['user_estado']).'\', \''.($tsData['user_sexo'] == 'm' ? 1 : 0).'\')');
			...
			$sexo = 'Bienvenid' . ($tsData['user_sexo'] == 'm' ? 'o' : 'a'); 
			...
			$body = '<div style="background:#0f7dc1;padding:10px;font-family:Arial, Helvetica,sans-serif;color:#000">
				...
				<div style="background:#FFF;padding:10px;font-size:14px">
					...
					<p>Antes de empezar a interactuar con la comunidad, te recomendamos que visites el <a target="_blank" href="http://'.$tsCore->settings['web'].'/pages/protocolo/">Protocolo</a> del sitio.</p>
			...
			$tsEmail->sendEmail($from, $to, $subject, $body)  or die('0: Hubo un error al intentar procesar lo solicitado');				
				...
				return '2: Bienvenido a <b>'.$tsCore->settings['titulo'].'</b>, Ahora estas registrado y tu cuenta ha sido activada, podr&aacute;s disfrutar de esta comunidad inmediatamente.<br><br>&iexcl;Muchas gracias! :)</div>';
			}
